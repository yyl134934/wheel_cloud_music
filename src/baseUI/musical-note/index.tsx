...
const MusicNote = forwardRef((props, ref) => {
  ...
  useEffect(() => {
    ...
    domArray.forEach((item: HTMLDivElement & { running: boolean }) => {
      item.running = false;
      item.addEventListener(
        'transitionend',
        function (this: HTMLDivElement & { running: boolean }) {
  ...
  const startAnimation = ({ x, y }: { x: number; y: number }) => {
    for (let i = 0; i < ICON_NUMBER; i++) {
      let domArray: (HTMLDivElement & { running: boolean })[] = [].slice.call(iconsRef.current?.children);
      let item = domArray[i];
      // 选择一个空闲的元素来开始动画
