...
function Album() {
  ...
  const nodeRef = useRef<any>(null); //进出动画ref
  const headerEl = useRef<any>(); //头部标题ref
  const musicalNoteRef = useRef<any>(); //音符动画ref

  const { currentSong } = usePlayingStore((state) => state.state);

  const { id } = useParams();

  const { isLoading, data: { playlist: currentAlbum = {} } = { playlist: {} } } = useQuery({
    queryKey: ['album', id],
      ...
      <TopDesc background={currentAlbum.coverImgUrl ?? ''}>
        ...
        <div className='img_wrapper'>
          ...
          <div className='play_count'>
            ...
            <span className='count'>{getCount(currentAlbum.subscribedCount ?? 0)}</span>
          </div>
        </div>
        ...
        <Scroll bounceTop={false} onScroll={handleScroll}>
          <div>
            ...
            <SongsList
              ...
              collectCount={currentAlbum.subscribedCount || 0}
              showCollect
